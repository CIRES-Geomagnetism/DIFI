...
import numpy as np
import math
from DIFI import legendre
from geomaglib import legendre as new_leg
#

def design_SHA_Sq_e_Re_v2(rho, theta, phi, nmax, mmax):
    ...
    if (np.size(phi, 0) != N_data):
        raise ValueError(
            "design_SHA_Sq_e: theta and phi have different dimensions"
        )
    ...
    Leg = new_leg.Flattened_Chaos_Legendre1(nmax, theta)
    ...
    for n in range(1, nmax+1):
        ...
        for m in range(Mmax+1):
            index = new_leg.get_index(n, m, nmax)
            if m == 0:
                # no h terms for g10, g20, g30 etc...
                A_r_0[k] = (-n)*rn1*Pnm[index][0]
                A_theta_0[k] = -rn1*dPnm[index][0]
                A_phi_0[k] = -rn1*0
                k += 1
            else:
                alpha = m*phi*rad
                ...
                A_r_0[k] = (-n)*rn1*Pnm[index][0]*cos_phi
                A_theta_0[k] = -rn1*dPnm[index][0]*cos_phi
                A_phi_0[k] = -rn1*Pnm[index][0]/sin_theta*(-m*sin_phi)
                k += 1
                # h terms as in h11, h21, h22 etc...
                A_r_0[k] = (-n)*rn1*Pnm[index][0]*sin_phi
                A_theta_0[k] = -rn1*dPnm[index][0]*sin_phi
                A_phi_0[k] = -rn1*Pnm[index][0]/sin_theta*m*cos_phi
                k += 1

    return A_r_0, A_theta_0, A_phi_0

def design_SHA_Sq_e_Re_v1(rho, theta, phi, nmax, mmax):
    # [A_r, A_theta, A_phi] = design_SHA_Sq_e_Re_v2(rho,theta,phi,t,t_ut,...
    #    nmax,mmax,p_vec,s_vec);
