...
def main():
    ...
    print("This is the SQ field for lat = 21.3166, lon = -157.9996, on November 1th, 2023")
    print(B)
    ...
    print( " ")
    print "This is a time series of the field for the 5th day of each month in 2023"
    print '\n' + str(B) + '\n'
...
def getSQfield(lat, lon, year, month, day, hour=0, minutes=0, h = 0):
    ...
    f107_1 = np.array([])
    for i in xrange(np.size(sq_t)):
        if sq_t[i] < start_time:
            raise Exception("Request before 2014.0 reached difi calculation improperly")
        elif sq_t[i] > end_time:
            raise Exception("Request after 2025.0 reached difi calculation improperly")
        while sq_t[i] < 5114.0:
            sq_t[i] += 365
        j = 0
        while difi_t_f107[j] < sq_t[i]:
            j += 1
        f107_1 = np.append(f107_1, difi_f107[j] * frac_arr[i] + difi_f107[j - 1] * (1 - frac_arr[i]))
    # index_f107 = [i for i in xrange(len(difi_t_f107)) if difi_t_f107[i] >= math.floor(sq_t) and difi_t_f107[i] <= math.ceil(sq_t)]
    # frac = sq_t-math.floor(sq_t)
    # f107_1 = np.multiply(difi_f107[index_f107[0]]*(1-frac)+difi_f107[index_f107[-1]]*(frac), np.ones(1))
    B_XYZ = {}
