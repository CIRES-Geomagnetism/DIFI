...
def gg2gm_2010(theta_gg = None, phi_gg = None, theta_gm = None, phi_gm = None, B_theta = None, B_phi = None, get_R = False):
    ...
    if (theta_gg is not None and phi_gg is not None) and (theta_gm is None or phi_gm is None):
        i_trans = 1
        theta_in = theta_gg
        phi_in = phi_gg
    elif (theta_gg is None and phi_gg is None) and (theta_gm is not None or phi_gm is not None):
        i_trans = -1
        theta_in = theta_gm
        phi_in = phi_gm
    else:
    ...
    A = np.matrix([[c_t_b*c_p_b, c_t_b*s_p_b,  -s_t_b], [-s_p_b, c_p_b, 0], [s_t_b*c_p_b, s_t_b*s_p_b, c_t_b]])
...
#Still need to pythonify the below...
    if B_theta is not None:
        raise NotImplementedError("The rotation of the Magnetic field vector is not implemented")
        BE=B_theta*c_t
        Bx=BE*c_p - B_phi*s_p
        By=BE*s_p + B_phi*c_p
        Bz= -B_theta*s_t
...
#Still need to pythonify the above
    ...
    if get_R:
        ...
        R_1[:,2,:]=np.matrix([c_t, -s_t, np.zeros(np.size(c_p))]).transpose()
        c_t=np.cos(theta_gm*rad)
        s_t=np.sin(theta_gm*rad)
        c_p=np.cos(phi_gm*rad)
        s_p=np.sin(phi_gm*rad)
        ...
        R_2[:,2,:]=np.matrix([-s_p, c_p, np.zeros(np.size(c_p))]).transpose()
