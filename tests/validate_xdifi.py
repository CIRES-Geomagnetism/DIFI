...
for i in range (0, len(data)):
    B = getSQfield.getSQfield(data[i]['lat'], data[i]['lon'], data[i]['year'], data[i]['month'], data[i]['day'], hour=data[i]['hour'], minutes = data[i]['min'], h = data[i]['r']-6371.2,f107_1 = data[i]['F10.7'])
    # if B['X'] != data[i]['X']:
