...
def MAG_PcupLow(x, nMax):
        ...
        for m in xrange(n+1):
            ...
            if n == m:
                ...
                if m > n - 2:
                    ...
                    k = np.float(((n - 1) * (n - 1)) - (m * m)) / np.float((2 * n - 1) * (2 * n - 3))
    ...
    for n in xrange(1,nMax+1):
        ...
        index1 = (n - 1) * n / 2
        # for m = 0
        schmidtQuasiNorm[index] = schmidtQuasiNorm[index1] * np.float((2 * n - 1)) / np.float(n)
        for m in xrange(1,n+1):
            index = (n * (n + 1) / 2 + m)
            index1 = (n * (n + 1) / 2 + m - 1)
            schmidtQuasiNorm[index] = schmidtQuasiNorm[index1] * np.sqrt(np.float((n - m + 1) * (2 if m == 1 else 1)) / np.float(n + m))
