function [B_1,B_2] = parforward_Sq_d_Re_v2(r,theta,phi,t,f107,s)
...
%
% Optimized version of "forward_Sq_d_Re_v2.m" with parallel for-loops (parfor).
%
% "v2" version: 
...
% calculate number of loops (parfor loop)

n_loops = ceil(N_data / chunk_size);

% reshape input and preallocate intermediate output (parfor loop)

zeros_data = zeros(chunk_size * n_loops - N_data, 1);

rho_s      = reshape([rho;      zeros_data], chunk_size, n_loops);
theta_s    = reshape([theta;    zeros_data], chunk_size, n_loops);
phi_s      = reshape([phi;      zeros_data], chunk_size, n_loops);
t_season_s = reshape([t_season; zeros_data], chunk_size, n_loops);
t_mut_s    = reshape([t_mut;    zeros_data], chunk_size, n_loops);

B_r_1_tmp     = zeros(chunk_size, n_loops);
B_theta_1_tmp = zeros(chunk_size, n_loops);
B_phi_1_tmp   = zeros(chunk_size, n_loops);

B_r_2_tmp     = zeros(chunk_size, n_loops);
B_theta_2_tmp = zeros(chunk_size, n_loops);
B_phi_2_tmp   = zeros(chunk_size, n_loops);   
...
if (min(rho) > rho_Sq)
    ...
    parfor k = 1:n_loops
        ...
        [theta_d, phi_d, R] = gg2gm_v2(theta_s(:, k), phi_s(:, k), ...
            1, [], [], [theta_NGP phi_NGP]);   % [deg]

        % calculate design matrices

        [A_r_i_d, A_theta_i_dd, A_phi_i_dd] = ...
            design_SHA_Sq_i_Re_v2(rho_s(:, k), theta_d, phi_d, ...
            t_season_s(:, k), t_mut_s(:, k), nmax, mmax, p_vec, s_vec);
        ...
        B_r_1_tmp(:, k)     = A_r_i_d' * m_e_d_Re_C;
        B_theta_1_tmp(:, k) = A_theta_i_d' * m_e_d_Re_C;
        B_phi_1_tmp(:, k)   = A_phi_i_d' * m_e_d_Re_C;
    
        B_r_2_tmp(:, k)     = A_r_i_d' * m_i_d_Re;
        B_theta_2_tmp(:, k) = A_theta_i_d' * m_i_d_Re;
        B_phi_2_tmp(:, k)   = A_phi_i_d' * m_i_d_Re;   
...
elseif (max(rho) < rho_Sq)
    ...
    parfor k = 1:n_loops
        ...
        [A_r_e_d, A_theta_e_dd, A_phi_e_dd] = ...
            design_SHA_Sq_e_Re_v2(rho_s(:, k), theta_d, phi_d, ...
            t_season_s(:, k), t_mut_s(:, k), nmax, mmax, p_vec, s_vec);
        ...
        B_r_1_tmp(:, k)     = A_r_e_d' * m_e_d_Re;
        B_theta_1_tmp(:, k) = A_theta_e_d' * m_e_d_Re;
        B_phi_1_tmp(:, k)   = A_phi_e_d' * m_e_d_Re;
...
B_r_1_tmp     = reshape(B_r_1_tmp,     n_loops * chunk_size, 1);
B_theta_1_tmp = reshape(B_theta_1_tmp, n_loops * chunk_size, 1);
B_phi_1_tmp   = reshape(B_phi_1_tmp,   n_loops * chunk_size, 1);

B_r_2_tmp     = reshape(B_r_2_tmp,     n_loops * chunk_size, 1);
B_theta_2_tmp = reshape(B_theta_2_tmp, n_loops * chunk_size, 1);
B_phi_2_tmp   = reshape(B_phi_2_tmp,   n_loops * chunk_size, 1);

B_r_1_tmp(N_data+1:end)     = [];
B_theta_1_tmp(N_data+1:end) = [];
B_phi_1_tmp(N_data+1:end)   = [];

B_r_2_tmp(N_data+1:end)     = [];
B_theta_2_tmp(N_data+1:end) = [];
B_phi_2_tmp(N_data+1:end)   = [];
